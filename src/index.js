...
function Scene({ top, mouse }) {
  ...
  return (
    <>
      ...
      <Text opacity={top.interpolate([0, 200], [1, 0])} position={top.interpolate(top => [0, -1 + top / 200, -0.3])}>
        PARKER
      </Text>
      <Text opacity={top.interpolate([0, 200], [1, 0])} position={top.interpolate(top => [0, 1 + top / 200, -1])}>
        MICHAEL
      </Text>
